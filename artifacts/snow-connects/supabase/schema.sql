...
create table if not exists bookings (
  ...
  bank_commission integer not null default 0,
...
alter table app_config add column if not exists bank_commission_rate  numeric not null default 0.04;
alter table app_config add column if not exists transaction_fee_kurus integer not null default 10000;
...
begin
  ...
  return json_build_object(
    ...
    -- Phase 7 splits revenue. The values below are written by the
    -- phase-7 admin_stats() override (kept here only for fresh-install
    -- defaults before that override runs).
    'customerPaidKurus', v_revenue_kurus,
    'revenueKurus', v_revenue_kurus,
    'bankCommissionKurus', 0,
    'transactionFeesKurus', 0,
...
-- create_booking REPLACED to honor test_mode: when on, sets payment_status
-- = 'paid' and is_test_booking = true, and returns both fields in the JSON
-- so the client can skip /payment.
create or replace function create_booking(
  p_instructor uuid,
  p_resort uuid,
  p_date date,
