-- Phase 6: fix payouts for test-mode auto-paid bookings.
--
-- Problem: when app_config.test_mode = true, create_booking() inserts the
-- booking with payment_status = 'paid' directly and skips the /payment
-- screen, which means confirm_payment() is never called and no row is
-- ever inserted into `payouts`. Result: customers see "Ödendi" but
-- instructors see no earnings.
--
-- Fix:
-- 1. Backfill payouts for any paid booking that is missing one.
-- 2. Update create_booking() to insert a payout row inline whenever
--    test_mode auto-pays the booking.
--
-- Idempotent: safe to re-run.

------------------------------------------------------------
-- 1. Backfill: paid bookings without a payout row
------------------------------------------------------------
insert into payouts (instructor_id, booking_id, gross_amount, commission,
                     net_amount, lesson_date, release_date, status)
select b.instructor_id,
       b.id,
       b.total_price,
       b.commission_amount,
       b.total_price - b.commission_amount,
       b.lesson_date,
       -- 21 business days after the lesson date, matching confirm_payment().
       (
         with recursive walk(d, added) as (
           select b.lesson_date::date, 0
           union all
           select (d + 1)::date,
                  added + case when extract(dow from d + 1) in (0, 6) then 0 else 1 end
             from walk
            where added < 21
         )
         select d from walk where added = 21 order by d limit 1
       ),
       'pending'
  from bookings b
...
------------------------------------------------------------
-- 2. create_booking(): create payout inline when test_mode auto-pays
------------------------------------------------------------
